...
void FsSimulation::SimDrawAllScreen(YSBOOL demoMode,YSBOOL showTimer,YSBOOL showTimeMarker) const
{
	...
	// End optimized frame rendering - simple optimizations
	static int endFrameCounter = 0;
	endFrameCounter++;

	// Periodic optimization checks
	if (endFrameCounter % 100 == 0)
	{
		// Force GL state cleanup every 100 frames
    ...
    const double &dt,const FsCockpitIndicationSet &cockpitIndicationSet,YSBOOL demoMode,YSBOOL showTimer,YSBOOL showTimeMarker,const ActualViewMode &actualViewMode) const
{
	...
	// Begin optimized frame rendering
	...
	class FsProjection &proj) const
{
	...
	{
		SimDrawAirplaneSafe(actualViewMode,proj,FSVISUAL_DRAWALL);
		SimDrawGroundSafe(actualViewMode,proj,FSVISUAL_DRAWALL);
	...
	{
		SimDrawAirplaneSafe(actualViewMode,proj,FSVISUAL_DRAWOPAQUE);
		SimDrawGroundSafe(actualViewMode,proj,FSVISUAL_DRAWOPAQUE);
		bulletHolder.BeginDraw();
		bulletHolder.Draw(
	...
	if(cfgPtr->drawTransparency==YSTRUE && cfgPtr->drawTransparentLater==YSTRUE)
	{
		SimDrawAirplaneSafe(actualViewMode,proj,FSVISUAL_DRAWTRANSPARENT);
		SimDrawGroundSafe(actualViewMode,proj,FSVISUAL_DRAWTRANSPARENT);
		bulletHolder.BeginDraw();
		bulletHolder.Draw(
...
void FsSimulation::SimDrawAirplane(const ActualViewMode &actualViewMode,const FsProjection &proj,unsigned int drawFlag) const
{
	...
	// Optimized airplane rendering with simple improvements
	int visibleCount = 0;
	...
	while((seeker=FindNextAirplane(seeker))!=NULL)
	{
		// Safety check: verify object is still valid and alive
		...
		// Simple distance-based LOD optimization with additional safety checks
		YsVec3 airPos;
		double airRad;
		double distance;
		...
		if(seeker!=NULL && seeker->IsAlive()==YSTRUE)
		{
			try {
				...
				// Handle ordinance rendering with optimizations
				if(cfgPtr->drawOrdinance==YSTRUE && distance < 5000.0 && seeker->IsAlive()==YSTRUE)
				{
				...
				// If drawing fails, skip this object silently
				continue;
			}
		}
			...
			// Handle ordinance rendering with optimizations
			if(cfgPtr->drawOrdinance==YSTRUE && distance < 5000.0) // Skip ordinance for very distant objects
			{
				seeker->Prop().DrawOrdinanceVisual(
				    drawCoarseOrdinance,seeker->weaponShapeOverrideStatic,actualViewMode.viewMat,proj.GetMatrix(),drawFlag);
			}
		...
		if((actualViewMode.actualViewMode==FSCOCKPITVIEW ||
		    actualViewMode.actualViewMode==FSADDITIONALAIRPLANEVIEW ||
		    actualViewMode.actualViewMode==FSADDITIONALAIRPLANEVIEW_CABIN) &&
		   IsPlayerAirplane(seeker)==YSTRUE && seeker!=NULL && seeker->IsAlive()==YSTRUE)
		{
			try {
	...
	//{
	//	printf("Aircraft drawn: %d/%d (frame %d)\n", visibleCount, GetNumAirplane(), drawCount);
	//}

	// Smoke and vapor must be drawn later due to transparency.
...
void FsSimulation::SimDrawGround(const ActualViewMode &actualViewMode,const FsProjection &proj,unsigned int drawFlag) const
{
	...
	// Optimized ground vehicle rendering with simple improvements
	...
	while((seeker=FindNextGround(seeker))!=NULL)
	{
		...
		// Enhanced distance and size-based culling with safety checks
		double objRad;
		double distance;
		double apparentRad;
		...
		if((actualViewMode.actualViewMode==FSCOCKPITVIEW ||
		    actualViewMode.actualViewMode==FSADDITIONALAIRPLANEVIEW ||
		    actualViewMode.actualViewMode==FSADDITIONALAIRPLANEVIEW_CABIN) &&
		   GetPlayerGround()==seeker && seeker!=NULL && seeker->IsAlive()==YSTRUE)
		{
			try {
				if(cockpit!=nullptr)
				{
	...
	// Handle aircraft carriers with optimization
	FsAircraftCarrierProperty::BeginDrawArrestingWire();
	for(int i=0; i<aircraftCarrierList.GetN(); i++)
	{
		if(aircraftCarrierList[i]!=NULL && aircraftCarrierList[i]->IsAlive()==YSTRUE)
		{
			try {
				...
				if(carrierDistance < 10000.0) // Only draw carriers within 10km
				{
					...
					// Only draw detailed bridge and wires if close enough
					if(carrierDistance < 3000.0)
					{
	...
	// Performance statistics (debug)
	{
		...
		{
			printf("Ground vehicles drawn: %d (frame %d)\n", visibleGroundCount, groundDrawCount);
		}
	}
}
